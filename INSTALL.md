...
Journeyman is designed for native Linux deployment with systemd. PostgreSQL is
recommended for operational installations. SQLite remains available for development,
evaluation, and small non-critical deployments.

For a new RHEL-family host, the supported happy path is the supplied Ansible installer.
The manual sections below document the same requirements so the installation can be
audited, adapted, or repaired without the playbook.
...
It installs the tested operating-system prerequisites, creates the Journeyman service
account and runtime directories, creates `/opt/journeyman/venv`, installs Python
requirements, configures a local PostgreSQL database with SCRAM authentication,
generates the credential-encryption key, applies database migrations, installs the
systemd units and Nginx apply helper, installs supplied TLS material, creates an HTTPS
bootstrap Nginx configuration, opens HTTP/HTTPS in firewalld when firewalld is active,
starts the services, generates the first break-glass administrator password, and runs
basic web/database checks.

The playbook deliberately does **not** issue certificates. Supply a private-key file and
a PEM file containing the complete certificate chain from the Ansible control host.

The play targets the inventory group `journeyman_servers`. If you are running the
installer directly on the future Journeyman server, use a local connection so Ansible
does not SSH back into the same machine:
...
If the playbook is being run from a separate Ansible control host, omit
`ansible_connection=local` and configure the normal SSH connection variables instead.

The installer expects the Journeyman source tree, including this playbook, to already be
staged at `/opt/journeyman` on the target. This is the normal clean-install workflow:
extract or check out Journeyman there, create the inventory, then run the supplied
playbook from that source tree.

Keep host-specific installer values in `host_vars`. For example, with this layout:

```text
inventory/
├── hosts.ini
└── host_vars/
    └── journeyman.example.com.yml
```

`inventory/host_vars/journeyman.example.com.yml` can contain:

```yaml
journeyman_fqdn: journeyman.example.com
journeyman_database_password: use-an-approved-secret-store
journeyman_tls_fullchain_src: /secure/path/journeyman-fullchain.pem
journeyman_tls_key_src: /secure/path/journeyman-key.pem
```

The TLS source paths are paths on the **Ansible control host**, not the target server.
Keep `journeyman_database_password` in Ansible Vault or another approved secret store
rather than committing it to Git.

If Python dependencies must be downloaded through a proxy, optionally add:

```yaml
journeyman_pip_proxy: http://proxy.example.com:8080
```

The proxy is passed directly to `pip` with `--proxy`; the installer also sets a 30-second
pip network timeout so an unreachable proxy fails instead of appearing to hang
indefinitely. If the proxy URL itself contains credentials, protect the variable with
Ansible Vault.

An example variable file is also provided at:

```text
deploy/ansible/install-journeyman.example.yml
```

Run the installer with the inventory directory so Ansible automatically loads its
`host_vars` file:

```bash
ansible-playbook -i inventory/hosts.ini deploy/ansible/install-journeyman.yml
```

If the target requires a different SSH user, add `-u bootstrap-admin` (and the normal
Ansible privilege-escalation options for the environment). Alternatively, variables may
be supplied explicitly with `-e @journeyman-install.yml`.

The playbook configures the source tree already staged at `/opt/journeyman`; it does not
copy a second application source tree. It is intended to be safe to rerun; persistent
secrets and the fallback administrator are not regenerated when their files already
exist.

When it completes, sign in with the displayed fallback administrator credential and
configure **Settings -> Directory and Authentication**. Verify both LDAPS servers and
directory searches before relying on AD login.
