...
RUNNER_CA_CERTIFICATE = Path("/etc/journeyman/runner-pki/ca-cert.pem")
...
""".format(
    ...
    # Runner-facing API requests use X.509 client authentication. Verification
    # is optional at the TLS layer so browsers and ordinary API clients continue
    # to work; /api/runners/* enforces SUCCESS in the application layer.
    ssl_client_certificate {runner_ca};
    ...
    location / {{
        ...
        proxy_set_header X-Journeyman-TLS-Client-Verify $ssl_client_verify;
        proxy_set_header X-Journeyman-TLS-Client-Cert $ssl_client_escaped_cert;
...
""".format(
            ...
            runner_ca=RUNNER_CA_CERTIFICATE,
        )
    )
...
def main():
    ...
    if not RUNNER_CA_CERTIFICATE.is_file():
        raise ApplyError(
            "Journeyman runner CA certificate is missing: {}".format(
                RUNNER_CA_CERTIFICATE
            )
        )
